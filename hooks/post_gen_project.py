...
pkg_name = (
    "{{ cookiecutter.package_import_name }}".replace(".", "_")
    .replace("-", "_")
    .lower()
)
...
with open(meta_yml_path, "r") as mfile:
    ...
    meta_yml_text = meta_yml_text.replace(
        "GENERATE_PYPI_URL", f"{pypi_source_URL}"
    )
    ...
    meta_yml_text = add_list(
        "{{ cookiecutter.build_requirements }}",
    ...
    meta_yml_text = add_list(
        "{{ cookiecutter.host_requirements }}",
    ...
    meta_yml_text = add_list(
        "{{ cookiecutter.runtime_requirements }}",
    ...
    meta_yml_text = add_list(
        "{{ cookiecutter.testing_requirements }}",
    ...
    # Add the maintainers
    meta_yml_text = add_list(
        "{{ cookiecutter.recipe_maintainers }}",
...
print(
    "Done! meta.yaml file has been created under the new {{ cookiecutter.package_import_name }} directory."
)
